...
<table class="table table-striped table-responsive-stack " id="tableOne{{$category->id}}">
    ...
    <tr>
        ...
        <th class="th-td-30">{{__('web/public.title')}}</th>
        ...
        <th class="th-td-20" >{{__('web/public.updated_at')}}</th>
    ...
    @foreach($category->products as $product)
        ...
        <tr>
            ...
            <td class="th-td-40"><a href="{{ route('web.show.product',$product->id) }}" class="title-link-in-list">{{\App\Providers\MyProvider::_text($product->title)}} </a></td>
            ...
            <td class="th-td-10" >{{empty($product->updated_at)? \App\Providers\MyProvider::show_date($product->created_at,'Y/n/j') : \App\Providers\MyProvider::show_date($product->updated_at,'Y/n/j')}}</td>
            <td class="th-td-16">@if($product->price==0) <a href="tel:{{\App\Providers\MyProvider::_text($siteDetailsProvider["phone_call_number"]->value)}}" target="_blank" rel="nofollow"  class="call-for-price">{{__('web/public.call_for_price')}}</a>
