            ...
            <div class="row clearfix">
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="body">
                            <form id="basic-form" action="{{ route('size.update',$size->id) }}" method="post"
                            ...
                            <?php
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="body_{{$kay}}" class="form-control ckeditor" rows="5" cols="30"
                                              required>{{\App\Providers\MyProvider::_text($size->body,$kay)}}</textarea>
                                    ...
                                    <div class="multiselect_div">
                                        <select id="single-selection" name="product_categories_id"
                                            ...
                                            @foreach($productCategories as $category)
                                                <option value="{{$category->id}}" {{($size->product_categories_id==$category->id)?"selected":""}}>{{\App\Providers\MyProvider::_text($category->title)}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="media">
                                    <img class="media-object " src="{{$size->images["thumb"]??null}}" alt="">
                                </div>
                                <div class="form-group">
                                    <label>{{__('admin/public.priority')}} :</label>
                                    <input type="number" name="priority" class="form-control"
                                           value="{{$size->priority}}" required>
                                </div>
                                ...
                                <div class="form-group col-lg-4 col-md-12">
                                    ...
                                    <div class="multiselect_div">
                                        <select id="single-selection" name="status"
                                            ...
                                            <option value="1" {{$size->status?"selected":""}}>{{__('admin/public.active')}}</option>
                                        </select>
                                    </div>
                                </div>
