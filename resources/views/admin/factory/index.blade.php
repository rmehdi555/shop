            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-5 col-md-8 col-sm-12">
                        <h2>{{__('admin/public.factory_list')}}</h2>
                    </div>
            ...
            <div class="row clearfix">
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="body">
                            <a href="{{ route('factory.create',['SID' => '202']) }}" class="btn btn-primary m-b-15">
                                <i class="icon wb-plus" aria-hidden="true"></i> {{__('admin/public.factory_add')}}
                            </a>
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                                    ...
                                    @foreach($factories as $factory)
                                        <tr class="gradeA">
                                            <td>{{$factory->id}}</td>
                                            <td>{{\App\Providers\MyProvider::_text($factory->title)}}</td>
                                            <td>{{$factory->status?__('admin/public.active'):__('admin/public.inactive')}}</td>
                                            <td class="actions">

                                                <form action="{{ route('factory.destroy', $factory->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('factory.edit',$factory->id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default m-r-5 button-edit"
