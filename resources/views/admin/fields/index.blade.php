            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-5 col-md-8 col-sm-12">
                        <h2>{{__('admin/public.fields_list')}}</h2>
                    </div>
            ...
            <div class="row clearfix">
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="body">
                            {{--<a href="{{ route('fields.create') }}" id="addToTable" class="btn btn-primary m-b-15" type="button">--}}
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                                    ...
                                    <tbody>
                                    @foreach($allField as $fields)
                                        <tr class="gradeA">
                                            <td>{{$fields->id}}</td>
                                            <td>{{\App\Providers\MyProvider::_text($fields->title)}}</td>
                                            <td>{{$fields->parent_id}}</td>
                                            <td>{{$fields->status?__('admin/public.active'):__('admin/public.inactive')}}</td>
                                            <td class="actions">

                                                <form action="{{ route('fields.destroy', $fields->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('fields.show',$fields->id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default m-r-5 button-show"
                                                       data-toggle="tooltip" data-original-title="{{__('admin/public.show')}}"><i class="icon-eye" aria-hidden="true"></i></a>
                                                    <a href="{{ route('fields.edit',$fields->id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default m-r-5 button-edit"
