            ...
            <div class="row clearfix">
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="body">
                            {{--<a href="{{ route('slider.create') }}" id="addToTable" class="btn btn-primary m-b-15" type="button">--}}
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                                    ...
                                    @foreach($allSlider as $slider)
                                        <tr class="gradeA">
                                            <td>{{$slider->id}}</td>
                                            <td>{{\App\Providers\MyProvider::_text($slider->title)}}</td>
                                            <td>{{$slider->type}}</td>
                                            <td>{{$slider->status?__('admin/public.active'):__('admin/public.inactive')}}</td>
                                            <td class="actions">

                                                <form action="{{ route('slider.destroy', $slider->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('slider.show',$slider->id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default m-r-5 button-show"
                                                       data-toggle="tooltip" data-original-title="{{__('admin/public.show')}}"><i class="icon-eye" aria-hidden="true"></i></a>
                                                    <a href="{{ route('slider.edit',$slider->id) }}" class="btn btn-sm btn-icon btn-pure btn-default on-default m-r-5 button-edit"
